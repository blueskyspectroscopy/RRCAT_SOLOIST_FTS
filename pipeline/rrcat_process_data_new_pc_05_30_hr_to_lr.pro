PRO rrcat_process_data_new_pc_05_30_hr_to_lr, RRCAT=RRCAT
  rt = 'Transmission'
  dateStr = '2018_05_30'
...
;    'Pyro_reference_trans_hi_res', $
    ;    'HEB_reference_trans_standard_res', $
    ; 'HEB_reference_trans_hi_res'];, $
...
;   'TES_PS_trans_standard_res']
  ...
  foreach dataDir, dataDirs do begin
    ...
    IF STRPOS(STRUPCASE(dataDir), 'REF') GE 0 THEN sample='Reference'
    ...
    foreach dataFile, dataFiles, fIndex do begin
      ...
      ENDELSE
      whOpd = WHERE(dat.opd GE -3.02500 AND dat.opd LE 3.02920, whCount)
      print, whCount
      print, res
      ...
      IF firstTime EQ 1 THEN BEGIN
        firstTime = 0
        opd = dat.opd[whOpd]
        sig = dat.signal[whOpd]
        avgSig = sig
      ENDIF ELSE BEGIN
        opd = [[opd], [dat.opd[whOpd]]]
        sig = [[sig], [dat.signal[whOpd]]]
        avgSig = avgSig + sig
      ENDELSE
    endforeach
...
;    avgSig = avgSig[whOpd]
    ...
    IF dateStr EQ '2018_05_30' THEN BEGIN
      IF det EQ 'TES' THEN BEGIN
        ...
        IF sample EQ 'Polystyrene' and res EQ 'Standard' then zpd_app = 0.0002
      ENDIF
      IF det EQ 'HEB' THEN BEGIN
      ...
      IF det EQ 'Pyro' THEN BEGIN
        ;zpd_app = -0.0016
        IF sample EQ 'Reference' AND res EQ 'Standard' then zpd_app = -0.0011
        IF sample EQ 'Reference' AND res EQ 'High' then zpd_app = -0.0011
      ENDIF
    ENDIF
    ...
    avgSpec = (FFT(shiftAvgSig, -1))[0:nPoints-1]*nPoints
    ...
    IF dateStr EQ '2018_05_30' THEN BEGIN
      IF det EQ 'TES' THEN BEGIN
        ...
        IF sample EQ 'PS' and res EQ 'Standard' then begin
          wnMin = 425
          wnMax = 560
        endif
    ...
    STOP
  endforeach
;  print, 1.20671*0.045497291
;  print, 1.20671*0.045621313
;  print, 1.20671*0.045464247
  ;  STOP
END
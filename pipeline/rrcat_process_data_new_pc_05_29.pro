PRO rrcat_process_data_new_pc_05_29, RRCAT=RRCAT
  rt = 'Reflection'
  rootDir = 'V:\RRCAT\Data\2018_05_28\'
  ;rootDir = 'U:\RRCAT\Data\2018_05_29\'
...
;      'Ref_trans']
  ...
  foreach dataDir, dataDirs do begin
    ...
    IF STRPOS(dataDirRoot, 'MCT') GT 0 THEN det='MCT'
    IF STRPOS(dataDirRoot, 'Pyro') GT 0 THEN det='Pyro'
    IF STRPOS(dataDir, 'Ref') EQ 0 THEN sample='Reference'
    ...
    foreach dataFile, dataFiles, fIndex do begin
      ...
      ;IF STRPOS(fBase, 'PS') LT 0 THEN continue
      print, fBase
    ...
    ENDIF else begin
      ...
      if det EQ 'TES' THEN BEGIN
        ...
        IF sample EQ 'Polystyrene' THEN yMax = 2.0
        IF sample EQ 'HDPE' THEN yMax = 12.0
      endif
      IF det EQ 'HEB' THEN BEGIN
        fact = -1.
        xr = [0, 100]
